 ...
 */
 ...
 if (pulseseq!=NULL) {
   ...
   tprm.step_usec=pulseseq->step;
   tprm.mpinc_usec=mpinc;
   tprm.mppul=mppul;
   index=pulseseq->index;
   ros_msg_add_var(smsg,&tprm,sizeof(struct SeqPRM),"prm","SeqPRM");
   ros_msg_add_var(smsg,&index,sizeof(int32),"index","int32");
   ros_msg_add_var(smsg,pulseseq->rep,sizeof(unsigned char)*tprm.len,"rep","unsigned char array");
   ros_msg_add_var(smsg,pulseseq->code,sizeof(unsigned char)*tprm.len,"code","unsigned char array");
   ros_msg_add_var(smsg,ptab,sizeof(int)*tprm.mppul,"ptab","int array");
   ros_msg_send(s, smsg);
   ros_msg_recv(s, rmsg);
